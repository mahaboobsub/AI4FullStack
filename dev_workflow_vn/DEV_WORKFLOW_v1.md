# BloodBridge AI — Developer Workflow & Execution Sequence (v1)

> **This is the file devs copy prompts from.** Prompts are arranged in the EXACT order to run them.
> Each prompt is tagged **[DEV 1]** or **[DEV 2]**. Stop at every 🏁 CHECKPOINT and merge to `main`.
...
4. Frontend is additive-only — wrap/extend, never rewrite working components.
5. Branches: `dev-1`, `dev-2`. Merge via PR. `git pull main` before each new prompt.
...
M6 [DEV2]
A3 [DEV1] ‖ A4 [DEV1] ‖ B3 [DEV2] ‖ B4 [DEV2] ─🏁 C5 (merge agents)
...
# STEP 1 — [DEV 1] Prompt A1 (run ALONE — Dev 2 does not code yet)
...
# STEP 2 — [DEV 1] Prompt A2  (parallel with B1, B2)
...
# STEP 3 — [DEV 2] Prompt B1  (parallel with A2)
...
IMPORTANT: import the LLM from core/llm_provider.get_fast_llm() (created in A1). Do NOT
instantiate ChatBedrock directly and do NOT edit core/config.py.
...
# STEP 4 — [DEV 2] Prompt B2  (after B1, parallel with A2)
...
NOTE: removing pytesseract/Pillow from requirements.txt is a SHARED-FILE change — Dev 2 lists
the packages to remove in the PR description; Dev 1 applies it to requirements.txt.
...
dict {blood_group, name, raw_text}.
...
# STEP 5 — [DEV 1] Prompt M1 (run ALONE — Dev 2 waits to start M4)
...
# STEP 6 — [DEV 1] Prompt M2  (parallel with M4)
...
# STEP 7 — [DEV 2] Prompt M4  (parallel with M2 — after C3)
...
# STEP 8 — [DEV 1] Prompt M3  (parallel with M5)
...
chains are filled or candidates exhausted). Add scipy to requirements.txt (Dev 1 owns it).
...
# STEP 9 — [DEV 2] Prompt M5  (parallel with M3 — needs M2 + B1)
...
EXTEND the EXISTING availability tool (set_my_availability / report_medical_hold) — do NOT add a
duplicate tool — so it accepts a reason + optional hold_until and calls the endpoint above. Wire it
so natural language like "I'm sick this week" or "got hospitalized, can't donate till 20th" triggers
it. Confirm warmly in the donor's language. (Plugs into B1's Bedrock tool loop.)
...
# STEP 10 — [DEV 2] Prompt M6  (additive UI — needs M4 + M5)
...
# STAGE 2 — Intelligence Agents (A3, A4 parallel with B3, B4)

# STEP 11 — [DEV 1] Prompt A3
...
to Bedrock Claude Haiku (via core/llm_provider.get_reasoning_llm()); get a plain-English 30-day
summary, specific shortage risks, and one recommended action per under-supplied blood type. Store
as agent_summary.
...
# STEP 13 — [DEV 2] Prompt B3  (parallel with A3/A4 — needs B1)
...
NOTE: B3 adds a job to scheduler/jobs.py (Dev 1-owned). Dev 2 writes the job function and gives it
to Dev 1 to register, OR appends a clearly-separated function for Dev 1 to wire at merge.
...
# STEP 14 — [DEV 2] Prompt B4  (parallel with A3/A4 — needs B1)
...
one Bedrock Claude Haiku call (core/llm_provider.get_reasoning_llm()) returns JSON: updated
tone_profile (warm/urgent/factual/inspirational), optional new emotional_anchor, optimal_contact_window,
best_channel, and a failure_reason label when outcome != CONFIRMED (e.g., wrong_channel, wrong_time,
message_too_long, fatigue_from_calls — the last maps to the real "limited activity despite multiple
calls" pattern). Update donor_memory accordingly (append anchors only if new).
...
# STAGE 3 — Frontend & Hardening (A5, A6 parallel with B5, B6)

# STEP 15 — [DEV 1] Prompt A5

```
CONTEXT:
Add a forecast section to dashboard/Admin.tsx. Additive only — do not modify existing components.
NOTE: A5 edits lib/api.ts which M4 also touched — pull main first and add a NEW function; do not
overwrite M4's location functions.
...
# STEP 17 — [DEV 2] Prompt B5  (parallel with A5/A6)
...
# STEP 18 — [DEV 2] Prompt B6  (parallel with A5/A6)

```
CONTEXT:
Responsible-AI requirement. Lock down sensitive tables to service_role and add a dedicated JWT secret.
NOTE: B6 needs JWT_SECRET added to core/config.py (Dev 1-owned). Dev 2 requests it in the PR; Dev 1
adds the field. B6 itself edits core/security.py only.
...
calls to the view.
TASK 5 — core/security.py: use JWT_SECRET env (64-char) exclusively for JWT sign/verify; stop using
...
→ /tmp/models/ if not cached; churn/urgency scorers load from S3 bucket "bloodbridge-models"
on cold start.
...
# STEP 21 — [DEV 2] Prompt B7  (parallel with A7/A8)
...
# STEP 22 — [DEV 2] Prompt B8  (after A7, A8, B7)
