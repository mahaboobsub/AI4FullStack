...
# Columns that exist on donors table (supabase_schema.sql) — antigen_data is NOT a column
DONOR_FLAG_COLUMNS = frozenset({
    "kell_negative", "duffy_negative", "kidd_negative",
    "rh_e_negative", "rh_c_negative", "mns_negative",
})
...
async def persist_ocr_results(donor_id: Optional[str], result: dict) -> None:
    ...
    for key, val in flags.items():
        if key in DONOR_FLAG_COLUMNS:
            update_payload[key] = val

    try:
        supabase.table("donors").update(update_payload).eq("donor_id", donor_id).execute()
        logger.info(f"Donor {donor_id} updated after OCR: blood_type={blood_group}, flags={list(flags.keys())}")
    except Exception as e:
        logger.error(f"Failed to update donor {donor_id} after OCR: {e}")

    # Store full antigen panel in donor_memory.notes (no antigen_data column on donors)
    if panel:
        try:
            panel_note = json.dumps({"antigen_panel": panel, "ocr_source": result.get("ocr_source", [])})
            ...
            logger.debug(f"donor_memory antigen notes skipped: {e}")
    ...
    try:
        supabase.table("donor_verifications").insert({
            ...
            "confidence": result.get("vision_confidence", 0.95 if blood_group else 0.5),
            "notes": json.dumps({
                "antigen_panel": panel,
                "antigen_flags": flags,
